...
var chaodaishuzu = ["春秋", "战国", "秦", "西汉", "东汉",
    "曹魏", "西晋",
    "东晋", "刘宋",
    "北魏", "东魏",
    "北齐", "隋代",
...
function init() {
    ...
    loadSelectEvent();
    oSpanNum = chaodaishuzu.indexOf(form_dynasty) + 1;
    changelayer();
...
function loadPopup_re(chaodairenwu) {
    ...
    allsource_global = all_Source;
    loadSelectEvent1(allsource_global);
    console.log(allsource_global)
    /**
     * 实现popup的html元素
     */
...
function loadSelectEvent1(all_Source) {
    ...
    dragBox.on('boxend', function () {
            ...
            all_Source.forEachFeatureIntersectingExtent(extent, function (feature) {
            ...
            url = '/search_tackle';
            var param = {}
            param.name = people_final.slice(-1).toString();
            console.log(people_final);
            $.post(url, param, function (res) {
                ...
                for (var i = 0; i < res.length; ++i) {
                    SearchName = res[i].name;//姓名
                    SearchInfo = res[i].info;//简介
                    SearchNames.push(SearchName);
                    SearchInfos.push(SearchInfo.substr(0, 33));
                    SearchName = '';
                    SearchInfo = '';
                }
                Static = '共选取：' + (SearchNames.length + 1).toString() + '人'
                $("#static").text(Static);
                console.log(SearchNames)
                var table = document.getElementById('SearchTable');//这是获取表
                // 移除上一个朝代的数据表格
                ...
                for (var i = 0; i < SearchNames.length; i++) {
                    ...
                    tr.innerHTML = '<td>' + (i + 1) + '</td>'
                        + '<td>' + SearchNames[i] + '</td>'
                        + '<td>' + SearchInfos[i] + '</td>'
                        + '<td>' + '<button class="GoToMap" id=' + SearchNames[i] + ' style="border-radius:12px;border:none;box-shadow: 0 3px 6px 0 rgba(0,0,0,0.1), 0 5px 10px 0 rgba(0,0,0,0.1);">' + '跳转' + '</button>' + '</td>';
...
// const infoBox = document.getElementById('status');
